...
export const GetModeratorMonitoringList = (managerId, isTutorAccount = true, search = "", page = 1, limit = 10000) => (dispatch) => {
  const url = `${process.env.REACT_APP_API_URL}moderationHistory/filter`;
  console.log('managerId', managerId)
  try {
    Axios.get(url, { params: { search, page, limit, managerId, isTutorAccount } }).then((res) => {
      if (res.data.status) {
