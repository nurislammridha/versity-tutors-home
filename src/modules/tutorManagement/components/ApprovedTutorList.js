...
const ApprovedTutorList = () => {
  ...
  const tutorManagementInformation = useSelector(
    (state) => state.tutorManagementInfo.profileList
  );
  const isUpdateLoading = useSelector(
    (state) => state.tutorManagementInfo.isUpdateLoading
  );
  ...
  const { result: profileList, totalPages } = tutorManagementInformation || { totalPages: 1 }
  ...
  useEffect(() => {
    const obj = { search, page, limit: 20, filters: { moderatorId: userInfo?._id, reviewStatus: "approved", isTutorAccount: true, categoryId, subCategoryId, divisionId, districtId, subDistrictId, areaId } }
    ...
    <>
      <div className="row align-items-center mb-4 p-3 bg-white rounded shadow-sm">
        <div className="col-md-4 mb-3 mb-md-3">
          <h4 className="mb-0 fw-semibold text-primary">Approved Tutor List</h4>
        </div>
...
export default ApprovedTutorList;
