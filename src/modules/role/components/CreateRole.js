...
import { AfterCreatedFalse, AllRolesByRoleType, SubmitRole } from "../_redux/RoleAction";
...
const CreateRole = () => {
  ...
  const isRole = useSelector(
    (state) => state.roleInfo.isRole
  );
  const afterCreated = useSelector(
    (state) => state.roleInfo.afterCreated
  );
  const handleSubmit = () => {
    const postData = {
